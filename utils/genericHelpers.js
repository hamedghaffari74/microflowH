...
export const getAllPathsFromStartToTarget = (
  startNodeId,
  targetNodeId,
  graph
) => {
...
const isHideRegisteredCredential = (params, paramsType, nodeFlowData) => {
  if (
    !nodeFlowData[paramsType] ||
    !nodeFlowData[paramsType]["credentialMethod"]
  )
    return undefined;
  ...
  for (let i = 0; i < clonedParams.length; i += 1) {
    ...
    if (input.type === "options") {
      const selectedCredentialMethodOption = input.options.find(
        (opt) => opt.name === nodeFlowData[paramsType]["credentialMethod"]
      );
      if (
